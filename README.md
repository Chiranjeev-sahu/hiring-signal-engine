...
GTM hiring signal engine built with n8n and Tavily that discovers careers pages, classifies hiring intent with AI, and finds verified hiring managers via a waterfall.
...
```mermaid
graph TD
    A[Tally Webhook<br>company + domain] --> B[Tavily Discover<br>Search + Extract]
    B --> C[Tavily Job Search<br>GTM roles]
    C --> D[LLM Classify<br>minimax 8 signals]
    D --> E{has_intent?}
    E -- No --> F[Quarantine<br>no_hiring_intent]
    E -- Yes --> G[Hunter Domain Search<br>limit 10]
    G --> H{Score >=100?}
    H -- Yes --> I[Prospeo Email<br>via LinkedIn URL]
    H -- No --> J[Tavily LinkedIn Search<br>site:linkedin.com/in]
    J --> K{Score >=0.75?}
    K -- No --> L[Quarantine<br>no_person_found]
    K -- Yes --> I
    I --> M[Dropcontact Fallback]
    M --> N[ZeroBounce Verify<br>api-us]
    N --> O{Valid?}
    O -- No --> P[Quarantine<br>email_not_verified]
    O -- Yes --> Q[HubSpot Upsert<br>+ Slack #all-fafo]
```
...
Live workflow WzvSZX5hAYXtZygU on Railway, webhook gtm-hiring-signal-v3.
...
`n8n` `Tavily` `LLM Classification` `Hunter` `Prospeo` `ZeroBounce` `HubSpot`
